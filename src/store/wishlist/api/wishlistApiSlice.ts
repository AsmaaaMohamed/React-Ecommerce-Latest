  ...
  endpoints: (builder) => ({
    getWishlist: builder.query({
      queryFn: async (dataType:TDataType) => {
        const { data, error } = await supabase.from("wishlist").select("productId");
        // console.log(data)
        if (error) {
    ...
    likeToggle:builder.mutation({
      queryFn: async(id:number)=>{
        const { data:isRecordExist,error } = await supabase.from('wishlist').select('*').eq('productId', id) ;
        if(error)
          return{error}
        if(isRecordExist?.length){
          ...
          .from('wishlist')
          .insert([
            { user_id:data?.user?.id , productId: id },
          ])
          .select();
          if (error)
            throw error
